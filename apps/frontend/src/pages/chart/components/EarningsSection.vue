<template>
    <div class="earnings-section" v-if="hasData">
        ...
        <div class="next-earnings" v-if="earnings.nextEarningsDate">
            <div class="icon-box">
                <f7-icon ios="f7:calendar" md="material:calendar_today" size="24" color="white"></f7-icon>
            </div>
            <div class="date-info">
        ...
        <div class="earnings-list" v-if="viewMode === 'revenue'">
            ...
            <div v-for="(item, i) in sortedRevenue" :key="i" class="list-row">
                ...
                <span class="earnings">{{ formatLargeNumber(item.earnings) }}</span>
            </div>
        </div>
    </div>
</template>
...
const sortedHistory = computed(() => {
    ...
    // Data earnings seringkali descending atau ascending tergantung API, pastikan konsisten
    // Log user: Array 0=Mar, 1=Jun... ini Ascending.
    // Biasanya kita ingin menampilkan yang terbaru di atas? Atau urutan normal?
    // Di log user urutannya Ascending (masa depan).
    // Tapi list tabel biasanya Descending (terbaru di atas).
    // Array user: history earnings (estimasi masa depan).
    // Mari kita balik agar yang terdekat tampil.
...
const sortedRevenue = computed(() => {
    ...
    // Log user: 1Q2025, 2Q2025... Ascending.
...
function formatPeriod(dateStr: string) {
    ...
    if (/^\d[Qq]\d{4}$/.test(dateStr)) {
        return dateStr.toUpperCase().replace(/(\d{4})/, " '$1").replace(" '20", " '"); // 1Q2025 -> 1Q '25
    }
...
.surprise.positive { color: #10b981; }
.surprise.negative { color: #ef4444; }

.revenue { color: #fff; font-weight: 500; }
.earnings { color: rgba(255,255,255,0.7); }
