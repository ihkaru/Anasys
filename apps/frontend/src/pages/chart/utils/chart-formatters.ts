...
function getTimezoneOffsetSeconds(targetTz: string): number {
  ...
  const targetHour = parseInt(formatter.format(now), 10);
  ...
  const estOffsetMin = targetTz === 'America/New_York' ? 
    (now.toLocaleString('en-US', { timeZone: targetTz, timeZoneName: 'short' }).includes('EDT') ? 240 : 300) : 
    0;
...
export function formatOHLCVForChart(data: OHLCVData[], timezoneMode: 'local' | 'exchange' = 'local'): ChartCandle[] {
  ...
  const offsetSeconds = timezoneMode === 'exchange' 
    ? getTimezoneOffsetSeconds('America/New_York')
    : 0;
