...
export const securityHeaders = new Elysia({ name: "securityHeaders" }).onAfterHandle(({ set }) => {
...
const getSafeHeaders = (headers: Headers) => {
	...
	if (auth) {
		safeHeaders.authorization = auth.startsWith("Bearer ") ? `Bearer ${auth.substring(7, 20)}...` : "[PRESENT]";
	}
