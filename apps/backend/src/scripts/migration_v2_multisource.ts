...
async function runMigration() {
    ...
    try {
        ...
        try {
            await db.execute(sql`ALTER TABLE "watchlist_items" ADD COLUMN IF NOT EXISTS "source" text DEFAULT 'YAHOO' NOT NULL`);
            ...
            try {
                ...
                await db.execute(sql`ALTER TABLE "watchlist_items" DROP CONSTRAINT IF EXISTS "watchlist_items_watchlist_id_symbol_id_pk"`);
                await db.execute(sql`ALTER TABLE "watchlist_items" ADD CONSTRAINT "watchlist_items_watchlist_id_symbol_id_source_pk" PRIMARY KEY ("watchlist_id", "symbol_id", "source")`);
