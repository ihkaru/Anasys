...
async function main() {
    ...
    for (let i = 0; i < targets.length; i += BATCH_SIZE) {
        ...
        const results = await Promise.allSettled(
            batch.map(sym => logoService.ensureLogo(sym.id, sym.ticker, sym.type))
        );
        ...
        if (batchNum % 100 === 0) {
            logger.info(`Progress: ${i + batch.length}/${targets.length} (${((i + batch.length) / targets.length * 100).toFixed(1)}%) - Success: ${successCount}, Failed: ${failCount}`);
        }
