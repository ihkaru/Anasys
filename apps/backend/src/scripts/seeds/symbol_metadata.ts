...
async function main() {
	...
	for (const row of records) {
		try {
			...
			const [existing] = await db.select().from(symbols).where(eq(symbols.ticker, row.ticker)).limit(1);
			...
			if (existing) {
				...
				await db.update(symbols).set(updateData).where(eq(symbols.id, existing.id)).execute();
				updated++;
