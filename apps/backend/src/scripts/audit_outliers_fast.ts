...
async function auditOutliersFast() {
  ...
  `);
    ...
    `);
		...
		logger.info(
			`   ✅ Deleted ${Number(result.count || crashCount[0]?.count).toLocaleString()} flash crashes in ${elapsed}s`,
		);
	}
  ...
  `);
    ...
    `);
		const elapsed = ((Date.now() - deleteStart) / 1000).toFixed(1);
		logger.info(
			`   ✅ Deleted ${Number(result.count || pumpCount[0]?.count).toLocaleString()} flash pumps in ${elapsed}s`,
		);
	}
