	...
	describe("7️⃣  Data Coverage Statistics", () => {
		it("should report coverage per symbol type", async () => {
			...
			result.forEach((r: any) => {
				console.log(
					`   ${r.type || "UNKNOWN"}: ${Number(r.symbol_count).toLocaleString()} symbols, ${Number(r.record_count).toLocaleString()} records`,
				);
				console.log(
					`      Range: ${r.earliest_date ? new Date(r.earliest_date).toISOString().split("T")[0] : "N/A"} to ${r.latest_date ? new Date(r.latest_date).toISOString().split("T")[0] : "N/A"}`,
				);
