...
export const watchlistController = new Elysia({ prefix: "/watchlists" })
	...
	.get("/:id", async ({ params, user }: any) => {
		...
		try {
			const watchlist = await watchlistService.getWatchlistWithItems(parseInt(params.id, 10), user.id);
			if (!watchlist) {
	...
	.patch(
		...
		async ({ params, body, user }: any) => {
			...
			try {
				const watchlist = await watchlistService.updateWatchlist(parseInt(params.id, 10), user.id, body);
				return { success: true, data: watchlist };
			} catch (e) {
	...
	.delete("/:id", async ({ params, user }: any) => {
		...
		try {
			await watchlistService.deleteWatchlist(parseInt(params.id, 10), user.id);
			return { success: true };
		} catch (e) {
