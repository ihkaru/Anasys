...
class MockYahooProvider extends YahooFinanceProvider {
	async fetchQuoteSummary(ticker: string, _modules: string[]) {
		...
		return {
			...
			earnings: {
				financialsChart: {
					quarterly: [{ date: "4Q2024", revenue: { raw: 250000000 }, earnings: { raw: 50000000 } }],
				},
			},
			recommendationTrend: {
				trend: [{ period: "0m", strongBuy: 10, buy: 5, hold: 2, sell: 0, strongSell: 0 }],
			},
			calendarEvents: {
...
describe("Financials Service Persistence", () => {
	...
	afterAll(async () => {
		...
		await db
			.delete(symbolFinancials)
			.where(
				eq(
					symbolFinancials.symbolId,
					db.select({ id: symbols.id }).from(symbols).where(eq(symbols.ticker, TEST_TICKER)),
				),
			);
		await db
			.delete(symbolEarnings)
			.where(
				eq(symbolEarnings.symbolId, db.select({ id: symbols.id }).from(symbols).where(eq(symbols.ticker, TEST_TICKER))),
			);
		await db
			.delete(analystRatings)
			.where(
				eq(analystRatings.symbolId, db.select({ id: symbols.id }).from(symbols).where(eq(symbols.ticker, TEST_TICKER))),
			);
