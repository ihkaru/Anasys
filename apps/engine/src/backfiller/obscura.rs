...
impl ObscuraFetcher {
    ...
    pub async fn search_symbol(&self, ticker: &str, expected_type: &str) -> Result<String> {
        ...
        let symbols = body["symbols"].as_array().ok_or_else(|| anyhow!("No symbols in search response"))?;

        if symbols.is_empty() {
            return Err(anyhow!("Symbol not found in TradingView search: {}", ticker));
        }

        // Search for a matching type first (e.g. stock, forex, crypto)
        let normalized_type = expected_type.to_lowercase();
        let best_match = symbols.iter().find(|s| {
    ...
    pub async fn fetch_candles(
        ...
        let canonical_symbol = match override_canonical {
            Some(c) => c,
            None => {
                match self.search_symbol(ticker, asset_type).await {
                    Ok(s) => {
        ...
        request.headers_mut().insert("Origin", HeaderValue::from_static("https://www.tradingview.com"));
        ...
        self.send(&mut ws, "set_auth_token", vec![json!("unauthorized_user_token")]).await?;
        sleep(Duration::from_millis(50)).await;
        
        self.send(&mut ws, "chart_create_session", vec![json!(chart_session), json!("")]).await?;
        sleep(Duration::from_millis(50)).await;

        let symbol_json = json!({ "symbol": canonical_symbol, "adjustment": "splits" }).to_string();
        self.send(&mut ws, "resolve_symbol", vec![
            json!(chart_session), 
            json!("sds_sym_1"), 
        ...
        self.send(&mut ws, "create_series", vec![
            json!(chart_session), 
            json!("sds_1"), 
            json!("s1"), 
            json!("sds_sym_1"), 
            ...
            match timeout(Duration::from_secs(10), ws.next()).await {
                Ok(Some(msg)) => {
                    ...
                    if text.starts_with("~h~") {
                        let _ = ws.send(Message::Text(format!("~m~{}~m~{}", text.len(), text).into())).await;
                        continue;
                    }
    ...
    fn parse_payload(&self, text: &str) -> Option<Value> {
        ...
        for part in parts {
            if part.is_empty() || part.chars().all(|c| c.is_ascii_digit()) || part.starts_with("~h~") { continue; }
