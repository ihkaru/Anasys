...
impl Backfiller {
    ...
    async fn process_single_task(&self, task: BackfillTask, batcher: Arc<Batcher>) -> Result<()> {
        ...
        if current_end <= start_ts {
            info!("  → Task {} ({}) finished. Reporting done.", task.ticker, task.interval);
            self.report_progress(task.id, &task.target_start_date, true, None).await?;
            return Ok(());
        }
        ...
        let result = if BinanceFetcher::is_binance_symbol(&task.ticker) {
            ...
            self.binance_fetcher.fetch_candles(&clean, &task.interval, current_start, current_end).await
        } else if use_obscura {
            ...
            let ws_res = {
                ...
                self.obscura_fetcher.fetch_candles(&task.ticker, &task.interval, &task.asset_type, override_tv, current_start, current_end).await
            };
            ...
            match ws_res {
                Ok((c, meta)) if !c.is_empty() => {
                    ...
                    if !c.is_empty() {
                        let count = c.len();
                        for candle in c { batcher.add_candle(candle).await?; }
                        batcher.flush_candles().await?;
                        info!("✅ Saved {} candles for {} ({}). Reporting progress with metadata...", count, task.ticker, task.interval);
                        self.report_progress(task.id, &task.target_start_date, false, meta).await?;
                ...
                _ => {
                    ...
                    self.yahoo_fetcher.fetch_candles(&task.ticker, &task.interval, current_start, current_end).await
                }
            }
        } else {
        ...
        match result {
            Ok(candles) => {
                if !candles.is_empty() {
                    ...
                    for c in candles { batcher.add_candle(c).await?; }
                    batcher.flush_candles().await?;
                    info!("✅ Saved {} candles for {} ({}). Reporting progress...", count, task.ticker, task.interval);
                    self.report_progress(task.id, &task.target_start_date, false, None).await?;
                } else {
            ...
            Err(e) => {
                error!("  → Fetch failed for {} ({}): {}", task.ticker, task.interval, e);
            }
        }
    ...
    async fn report_progress(&self, task_id: i32, last_date: &str, done: bool, metadata: Option<serde_json::Value>) -> Result<()> {
        let client = reqwest::Client::new();
        let url = format!("{}/market/internal/backfill/report", self.api_url);
        
        debug!("📤 Sending progress to {}: task_id={}, done={}, has_metadata={}", url, task_id, done, metadata.is_some());
        ...
        if resp.status().is_success() {
            ...
            warn!("⚠️ Progress report FAILED for task {}: {} - {}", task_id, last_date, err_body);
        }
        Ok(())
    }
}
