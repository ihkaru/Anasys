...
async function check() {
	...
	try {
		...
		console.log(
			"1. Column 'source':",
			cols.length > 0 ? "EXISTS" : "MISSING",
			cols.map((c) => ({ name: c.column_name, type: c.data_type, null: c.is_nullable })),
		);
	} catch (e) {
		console.log("Check column failed:", e);
	}
	...
	try {
		...
		console.log("Check nulls failed:", e);
	}
	...
	try {
		...
		console.log("Check duplicates failed:", e);
	}
	...
	try {
		...
		console.log(
			"4. Constraints:",
			cons.map((c) => ({ name: c.conname, type: c.contype })),
		);
	} catch (e) {
		console.log("Check constraints failed:", e);
	}
