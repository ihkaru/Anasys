...
export class CandleService {
	...
	async getOHLCV(ticker: string, interval = "1d", limit = 100, before?: string, source: string = "YAHOO") {
		...
		if (!beforeDate) {
			...
			if (isStale) {
				this.logger.info(
					`[getOHLCV] Data stale for ${ticker} (${interval}) via ${source}. Last: ${lastTimestamp?.toISOString() || "NEVER"}. Syncing...`,
				);
				try {
		...
		this.logger.info(
			`[getOHLCV] Query DB for ${ticker} (${interval}) via ${source} returned ${candles.length} candles` +
		...
		if (candles.length === 0 && beforeDate) {
			...
			try {
				...
				this.logger.info(
					`[getOHLCV] Backfill query took ${Date.now() - startQuery}ms. Retrieved ${candles.length} candles.`,
				);
			} catch (e) {
	...
	private isStale(lastDate: Date | null, interval: string): boolean {
		...
		switch (interval) {
			case "1m":
				return diffMinutes > 5; // 5 min old
			case "5m":
				return diffMinutes > 10;
			case "15m":
				return diffMinutes > 20; // Allow 20 min lag
			case "30m":
				return diffMinutes > 40;
			case "1h":
				return diffMinutes > 75; // 1h 15m
			case "4h":
				return diffMinutes > 260;
			case "1d":
				return diffMinutes > 1600; // > 26 hours (allow for next day market open)
			case "1wk":
				return diffMinutes > 10080 + 1440; // 1 week + 1 day
			default:
