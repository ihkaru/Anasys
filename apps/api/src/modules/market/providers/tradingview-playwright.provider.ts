...
import type { IDataProvider, QuoteResult, SearchResult, UnifiedCandle, TrendingResult } from "./data-provider.interface";
...
export class TradingViewPlaywrightProvider implements IDataProvider {
  ...
  async fetchChart(ticker: string, options: any): Promise<UnifiedCandle[]> {
    ...
    try {
      ...
      const data = await page.evaluate(async ({ symbol, res, range }) => {
